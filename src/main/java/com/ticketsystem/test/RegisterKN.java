...
public class RegisterKN {
	...
	static Logger log = LogManager.getLogger(RegisterKN.class);
		...
		try {
			...
			while(true) {
				try {
					...
					log.info("==========["+format.format(nowDate)+"]循环内，系统运行出错==========");
					e2.printStackTrace();
				} finally {
					registerCount++;
					Date nowDate = new Date();
					SimpleDateFormat format = new SimpleDateFormat("yyyy-MM-dd HH:mm:ss");
					log.info("！！！！！！！！！！！！！！！！！！！！！！！！！！！！！！！！！！！！！！！！");
					log.info("！！！！！！！！！！["+format.format(nowDate)+"]程序循环了"+registerCount+"次！！！！！！！！！！");
					log.info("！！！！！！！！！！！！！！！！！！！！！！！！！！！！！！！！！！！！！！！！");
				}
			}
		}catch(Exception e) {
			Date nowDate = new Date();
			SimpleDateFormat format = new SimpleDateFormat("yyyy-MM-dd HH:mm:ss");
			log.info("==========["+format.format(nowDate)+"]系统运行出错==========");
			e.printStackTrace();
		}
	}
	...
	public void start() {
		...
		*/
		...
		log.info("==========获取手机号成功:"+phoneNo+"==========");
		
		
		//检查手机号是否已经注册
		log.info("==========检查手机号是否已经注册==========");
		...
		log.info(ssName+ssName2+ssName3);
		String firstName = "张";
		String lastName = "三";
		try {
		...
		log.info("==========获取客户名称成功:"+ssName+ssName2+ssName3+"==========");
		
		
		String sessionStr = "";
		try {
			//获取官网session
			log.info("==========获取官网session==========");
			...
			log.info("==========获取官网session成功:"+sessionStr+"==========");
		...
		if (massgeStatus == null || massgeStatus.length()<2||!"ok".equalsIgnoreCase(massgeStatus)) {
			...
			if("短信还未到达,请继续获取".equalsIgnoreCase(massgeStatus)) {
				log.info("短信还未到达,第二次等待10秒，继续获取！");
				//等待手机接收验证码
				try {
					log.info("第二次等待10秒，等待手机接收到验证码！");
					int sleepTime = 0;
					while(true) {
				...
				if (massgeStatus == null || massgeStatus.length()<2||!"ok".equalsIgnoreCase(massgeStatus)) {
					log.info("获取手机验证码失败！");
					if("短信还未到达,请继续获取".equalsIgnoreCase(massgeStatus)) {
						log.info("短信还未到达,第三次等待10秒，继续获取！");
						//等待手机接收验证码
						try {
							log.info("第三次等待10秒，等待手机接收到验证码！");
							int sleepTime = 0;
							while(true) {
								if(sleepTime==10) {break;}
								log.info(sleepTime+"s");
						...
						if (massgeStatus == null || massgeStatus.length()<2||!"ok".equalsIgnoreCase(massgeStatus)) {
							log.info("获取手机验证码失败！");
							if("短信还未到达,请继续获取".equalsIgnoreCase(massgeStatus)) {
								log.info("短信还未到达,第四次等待10秒，继续获取！");
								//等待手机接收验证码
								try {
									log.info("第四次等待10秒，等待手机接收到验证码！");
									int sleepTime = 0;
									while(true) {
										if(sleepTime==10) {break;}
										log.info(sleepTime+"s");
								...
								if (massgeStatus == null || massgeStatus.length()<2||!"ok".equalsIgnoreCase(massgeStatus)) {
									log.info("获取手机验证码失败！");
									if("短信还未到达,请继续获取".equalsIgnoreCase(massgeStatus)) {
										log.info("短信还未到达,第五次等待10秒，继续获取！");
										//等待手机接收验证码
										try {
											log.info("第五次等待10秒，等待手机接收到验证码！");
											int sleepTime = 0;
											while(true) {
												if(sleepTime==10) {break;}
												log.info(sleepTime+"s");
										...
										if (massgeStatus == null || massgeStatus.length()<2||!"ok".equalsIgnoreCase(massgeStatus)) {
											log.info("获取手机验证码失败！");
											if("短信还未到达,请继续获取".equalsIgnoreCase(massgeStatus)) {
												log.info("短信还未到达,第六次等待10秒，继续获取！");
												//等待手机接收验证码
												try {
													log.info("第六次等待10秒，等待手机接收到验证码！");
													int sleepTime = 0;
													while(true) {
														if(sleepTime==10) {break;}
														log.info(sleepTime+"s");
												...
												if (massgeStatus == null || massgeStatus.length()<2||!"ok".equalsIgnoreCase(massgeStatus)) {
													log.info("获取手机验证码失败！");
													if("短信还未到达,请继续获取".equalsIgnoreCase(massgeStatus)) {
														log.info("短信还未到达,第七次等待10秒，继续获取！");
														//等待手机接收验证码
														try {
															log.info("第七次等待10秒，等待手机接收到验证码！");
															int sleepTime = 0;
															while(true) {
																if(sleepTime==10) {break;}
																log.info(sleepTime+"s");
														...
														if (massgeStatus == null || massgeStatus.length()<2||!"ok".equalsIgnoreCase(massgeStatus)) {
															log.info("获取手机验证码失败！");
															if("短信还未到达,请继续获取".equalsIgnoreCase(massgeStatus)) {
																log.info("短信还未到达,第八次等待10秒，继续获取！");
																//等待手机接收验证码
																try {
																	log.info("第八次等待10秒，等待手机接收到验证码！");
																	int sleepTime = 0;
																	while(true) {
																		if(sleepTime==10) {break;}
																		log.info(sleepTime+"s");
																...
																if (massgeStatus == null || massgeStatus.length()<2||!"ok".equalsIgnoreCase(massgeStatus)) {
																	log.info("获取手机验证码失败！");
																	if("短信还未到达,请继续获取".equalsIgnoreCase(massgeStatus)) {
																		log.info("短信还未到达,第九次等待10秒，继续获取！");
																		//等待手机接收验证码
																		try {
																			log.info("第九次等待10秒，等待手机接收到验证码！");
																			int sleepTime = 0;
																			while(true) {
																				if(sleepTime==10) {break;}
																				log.info(sleepTime+"s");
	...
	public void sendValiate(String sessionStr, String phoneNo) {
		...
		if("failed".equalsIgnoreCase(this.sendStatus)) {
			...
			//准备再来一次
			try {
				log.info("==========第"+this.getValiateCount+"次获取验证码==========");
				log.info("循环获取随机码失败，等待20秒，再次获取随机码！");
				int sleepTime = 0;
				while(true) {
					if(sleepTime==20) {break;}
					log.info(sleepTime+"s");
